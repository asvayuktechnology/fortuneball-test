...
export const TablePagination = (props: Props) => {
    const { page, perPage, totalCout, handdePageChange } = props;
    const totalPages = Math.ceil(totalCout / perPage);
    const handdePageMove = (type: 'next' | 'pre') => {
        let updatedPage = 1;
        if (type === 'next') {
            if (page < totalPages) {
                updatedPage = Math.max(page + 1, 1);
                handdePageChange(updatedPage);
            }
        }
        if (type === 'pre') {
            if (page > 0) {
                ...
                <div className="">
                    <p className="text-sm text-slate-200">
                        ...
                        <span className="font-medium ml-2 mr-2">
                            {totalCout === 0 ? 0 : (page - 1) * perPage + 1}
                        </span>
                        to
                        <span className="font-medium ml-2 mr-2">
                            {Math.min(page * perPage, totalCout)}
                        </span>
                        of
                        <span className="font-medium ml-2 mr-2">{totalCout}</span>
                        records
                    </p>
